...
export function isSnakeChunk(go: GameObject): go is SnakeChunk {
  return (go as SnakeChunk).name == "snake-chunk";
}

export function isSnakeHead(go: GameObject): go is SnakeHead {
  return isSnakeChunk(go) && (go as SnakeHead).direction != undefined;
}
...
export function crashedWithWorld(
  head: SnakeHead,
